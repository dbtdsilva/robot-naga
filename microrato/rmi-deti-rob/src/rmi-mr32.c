...
// This is an aproximation to calculate the distance measured by a Sharp GP2D12 sensor
...
void readAnalogSensors(void)
{
   ...
   {	
      ...
      switch(i)
      {
         ...
         case 1:
         case 2:
            adcValue = adcValue < 147 ? 147 : adcValue;
            int filteredValue = average(i, median(i, adcValue));
            analogSensors.array[i] = 6200 / (filteredValue - 70);  // distance in centimeters
            break;
