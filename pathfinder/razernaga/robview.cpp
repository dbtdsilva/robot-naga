...
SensorObstItem::SensorObstItem(double posx, double posy, double dir, double un)
                               : QGraphicsEllipseItem(posx,posy,0, 0, 0)
{
...
RobScene::RobScene(double irSensorAngles[NUM_IR_SENSORS])
{
    ...
    QGraphicsEllipseItem *rob = new QGraphicsEllipseItem(rob_x, rob_y, rob_radius * 2, rob_radius * 2, 0);
    this->addItem(rob);
    rob->setBrush(QBrush("Blue"));
    rob->setVisible(true);

    compassLine = new QGraphicsLineItem(0);
    ...
    for(int b=0; b<GetNumberOfBeacons();b++) {
        QGraphicsLineItem *tmp = new QGraphicsLineItem(0);
        beaconLine.push_back(tmp);
        this->addItem(tmp);
    ...
    for(int i=0; i < NUM_IR_SENSORS; i++) {
        sensorObst[i]=new SensorObstItem((rob_x + rob_radius) - rob_radius*sin(irSensorAngles[i]/180.0*M_PI),
                                         (rob_y + rob_radius) - rob_radius*cos(irSensorAngles[i]/180.0*M_PI),
                                         irSensorAngles[i],unit);
        this->addItem(sensorObst[i]);
   }
